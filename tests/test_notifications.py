...
SMTP_HOST = os.getenv(
    "HONEYGRID_SMTP_HOST", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_SMTP_HOST")
)
SMTP_PORT = os.getenv(
    "HONEYGRID_SMTP_PORT", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_SMTP_PORT", "587")
)
SMTP_USERNAME = os.getenv(
    "HONEYGRID_SMTP_USERNAME", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_SMTP_USERNAME")
)
SMTP_PASSWORD = os.getenv(
    "HONEYGRID_SMTP_PASSWORD", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_SMTP_PASSWORD")
)
SMTP_FROM = os.getenv(
    "HONEYGRID_SMTP_FROM", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_FROM_ADDRESS")
)
SMTP_TO = os.getenv(
    "HONEYGRID_SMTP_TO", os.getenv("HONEYGRID_NOTIFICATIONS_EMAIL_TO_ADDRESSES")
)
...
async def test_discord_notifier():
    ...
    if not RUN_LIVE:
        if _skip(
            "Live Discord webhook test disabled. Set HONEYGRID_RUN_LIVE_NOTIFICATIONS=1 to enable."
        ):
            return
    webhook_url = os.getenv(
        "HONEYGRID_DISCORD_WEBHOOK",
        os.getenv("HONEYGRID_NOTIFICATIONS_DISCORD_WEBHOOK_URL"),
    )
    if not webhook_url:
...
def test_severity():
    ...
    for event_type, expected_severity in test_cases:
        ...
        print(
            f"   {status} {event_type} → {severity.value} (expected: {expected_severity.value})"
        )
...
def test_notification_config():
    ...
    config = NotificationConfig(
        enabled=True, rate_limit_seconds=30, min_severity=Severity.MEDIUM
    )
...
async def test_email_notifier():
    ...
    if not RUN_LIVE:
        if _skip(
            "Live SMTP test disabled. Set HONEYGRID_RUN_LIVE_NOTIFICATIONS=1 to enable."
        ):
            return
    ...
    if not SMTP_HOST or not SMTP_FROM or not SMTP_TO:
        if _skip(
            "Missing SMTP settings. Set HONEYGRID_SMTP_HOST/SMTP_FROM/SMTP_TO to enable."
        ):
            return
