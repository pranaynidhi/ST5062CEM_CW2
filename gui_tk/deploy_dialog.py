...
class DeployDialog:
    ...
    def __init__(
        self, parent, db_manager, on_deploy_callback: Optional[Callable] = None
    ):
        """
    ...
    def _build_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="Deploy Honeytoken", font=("Segoe UI", 14, "bold")
        )
        ...
        # Token ID
        ttk.Label(token_frame, text="Token ID:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.token_id_var = tk.StringVar(
            value=f"token-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )
        token_id_entry = ttk.Entry(
            token_frame, textvariable=self.token_id_var, width=40
        )
        token_id_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=5, padx=(5, 0))

        # Token Name
        ttk.Label(token_frame, text="Token Name:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.token_name_var = tk.StringVar(value="Secret Document")
        token_name_entry = ttk.Entry(
            token_frame, textvariable=self.token_name_var, width=40
        )
        token_name_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5, padx=(5, 0))

        # Token Type
        ttk.Label(token_frame, text="Token Type:").grid(
        ...
        # Token Path
        ttk.Label(token_frame, text="File Path:").grid(
        ...
        browse_btn = ttk.Button(
            path_frame, text="Browse...", command=self._browse_file, width=10
        )
        ...
        self.agent_listbox = tk.Listbox(
            list_frame, yscrollcommand=scrollbar.set, selectmode=tk.SINGLE, height=8
        )
        ...
        self.agent_info_label = ttk.Label(
            agent_frame, text="Select an agent from the list above", foreground="gray"
        )
        ...
        options_frame = ttk.LabelFrame(
            main_frame, text="Deployment Options", padding="10"
        )
        ...
        cancel_btn = ttk.Button(
            button_frame, text="Cancel", command=self._cancel, width=15
        )
        cancel_btn.pack(side=tk.RIGHT, padx=(0, 5))

        deploy_btn = ttk.Button(
            button_frame, text="Deploy", command=self._deploy, width=15
        )
    ...
    def _load_agents(self):
        ...
        try:
            ...
            for agent in sorted(
                agents, key=lambda a: (a["status"] != "healthy", a["agent_id"])
            ):
            ...
            messagebox.showerror(
                "Error", f"Failed to load agents: {str(e)}", parent=self.dialog
            )
    ...
    def _validate_inputs(self) -> bool:
        ...
        if not self.token_id_var.get().strip():
            messagebox.showerror(
                "Validation Error", "Token ID is required", parent=self.dialog
            )
            return False
        ...
        if not self.token_name_var.get().strip():
            messagebox.showerror(
                "Validation Error", "Token name is required", parent=self.dialog
            )
            return False
        ...
        if not self.token_path_var.get().strip():
            messagebox.showerror(
                "Validation Error", "File path is required", parent=self.dialog
            )
            return False
        ...
        if not selection:
            messagebox.showerror(
                "Validation Error", "Please select a target agent", parent=self.dialog
            )
            return False
    ...
    def _deploy(self):
        ...
        selected_text = self.agent_listbox.get(selection[0])
        agent_id = (
            selected_text.split(" - ")[0].replace("● ", "").replace("○ ", "").strip()
        )
