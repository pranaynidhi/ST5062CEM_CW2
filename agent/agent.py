...
def main():
    ...
    parser.add_argument("--server-host", required=True, help="Server hostname/IP")
    parser.add_argument(
        "--server-port", type=int, default=9000, help="Server port (default: 9000)"
    )

    # Certificates
    parser.add_argument("--ca-cert", default="certs/ca.crt", help="CA certificate path")
    parser.add_argument(
        "--client-cert",
    ...
    if args.config:
        ...
        token_ids = args.token_ids or [
            f"token-{i:03d}" for i in range(len(watch_paths))
        ]
