...
class SecureSender:
    ...
    def __init__(
        ...
        # Statistics
        self.stats = {"sent": 0, "failed": 0, "rate_limited": 0, "reconnects": 0}
    ...
    def _create_ssl_context(self) -> ssl.SSLContext:
        ...
        context.load_cert_chain(
            certfile=str(self.client_cert_path), keyfile=str(self.client_key_path)
        )
    ...
    def send_event(
        self, token_id: str, path: str, event_type: str, **extra_fields
    ) -> bool:
        """
        ...
        try:
            msg = create_event_message(
                self.agent_id, token_id, path, event_type, **extra_fields
            )

            success = self._send_message(msg, rate_limit=True)

            if success:
                logger.info(f"📤 Sent event: {event_type} - {path} (token: {token_id})")
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(
            f"   Request {i+1}: {'✓' if acquired else '✗'} (tokens: {limiter.get_tokens():.2f})"
        )
    ...
    if not cert_dir.exists():
        ...
        if all(p.exists() for p in [ca_cert, client_cert, client_key]):
            ...
            print(
                f"   Connection: {'✓ Success' if connected else '✗ Failed (expected)'}"
            )
