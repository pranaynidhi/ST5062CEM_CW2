...
def apply_env_overrides(
    config: Dict[str, Any], prefix: str = "HONEYGRID"
) -> Dict[str, Any]:
    """
...
DEFAULT_SERVER_CONFIG = {
    "server": {
        ...
        "database": {"path": "data/honeygrid.db", "password": "change_this_password"},
        "security": {
...
DEFAULT_AGENT_CONFIG = {
    "agent": {
        ...
        "monitoring": {
            "watch_paths": [],
            "rate_limit": {"max_events_per_second": 10, "burst_size": 20},
        },
        "heartbeat": {"interval_seconds": 30, "timeout_seconds": 10},
    },
    "logging": {"level": "INFO"},
}
...
if __name__ == "__main__":
    ...
    print(
        f"   Notifications enabled: {get_nested_value(server_config, 'notifications.enabled')}"
    )
    ...
    print(
        f"   Server: {get_nested_value(agent_config, 'agent.server.host')}:{get_nested_value(agent_config, 'agent.server.port')}"
    )
