        ...
        try:
            ...
            if len(nonce_bytes) != NONCE_SIZE:
                raise ValidationError(
                    f"Invalid nonce size: {len(nonce_bytes)} (expected {NONCE_SIZE})"
                )
...
class Message:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        return {"header": self.header.to_dict(), "data": self.data}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "Message":
        """Create from dictionary."""
        return cls(header=MessageHeader.from_dict(data["header"]), data=data["data"])
...
def create_message(
    agent_id: str, msg_type: str, data: Dict[str, Any], timestamp: Optional[int] = None
) -> Message:
    """
...
async def read_frame(reader) -> bytes:
    ...
    if len(payload) < message_length:
        raise FramingError(
            f"Incomplete message: expected {message_length}, got {len(payload)}"
        )
...
def create_event_message(
    agent_id: str, token_id: str, path: str, event_type: str, **extra_fields
) -> Message:
    """
...
def create_heartbeat_message(agent_id: str, status: str = "healthy") -> Message:
    ...
    Returns:
        Heartbeat Message
    """
    data = {"status": status, "uptime": time.time()}
