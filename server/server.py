    ...
    def __init__(
        ...
        if ssl_object:
            ...
            if peer_cert:
                ...
                logger.info(
                    f"Client connected: {self.agent_id} from {addr[0]}:{addr[1]}"
                )
...
class HoneyGridServer:
    ...
    def _init_notifiers(self) -> list:
        ...
        base_config = NotificationConfig(
            ...
            batch_interval_seconds=notif_config_dict.get(
        ...
        if email_config.get("enabled", False):
            try:
                ...
                logger.info(
                    f"✓ Email notifications enabled ({len(email_notifier.to_addresses)} recipients)"
                )
            except Exception as e:
                ...
                for agent in agents:
                    ...
                    if time_since_seen > self.agent_timeout:
                        ...
                        if agent.get("status") == "healthy":
                            logger.info(
                                f"Agent {agent['agent_id']} entering warning state"
                            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {**self.stats, **db_stats, "nonce_cache_size": self.nonce_cache.size()}
...
async def main():
    ...
    db_path = args.db or get_nested_value(
        config, "server.database.path", "data/honeygrid.db"
    )
    db_password = args.db_password or get_nested_value(
        config, "server.database.password", "change_this_password"
    )

    # Create server
    server = HoneyGridServer(
        host=host, port=port, db_path=db_path, db_password=db_password, config=config
    )
